...
import { Analytics } from "@vercel/analytics/react" // FIX: Changed from /next to /react
import { 
  Github, 
  Brain, 
  Loader2, 
...
const RepoInterviewTool = () => {
  ...
  const [interviewData, setInterviewData] = useState(null); // FIX: Changed from '= null)' to '= useState(null)'
...
`).join('\n')}
  ...
  return (
    <> {/* FIX: Added React Fragment to contain Analytics and the main div */}
      <Analytics /> {/* <--- PLACED ANALYTICS HERE */}
          ...
          {step === 'file-select' && (
            <div className="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4">
              <div className={`border rounded-2xl overflow-hidden shadow-2xl backdrop-blur-xl ${isDark ? 'bg-slate-900/80 border-slate-800' : 'bg-white/70 border-white/50'}`}>
                  <div className={`p-6 border-b flex justify-between items-center ${isDark ? 'border-slate-800 bg-slate-950/50' : 'bg-slate-100 bg-slate-50/50'}`}>
                    <div>
